...
const CONTRACT_FIELDS: Array<{ field: string; type: string; description: string }> = [
  ...
  { field: 'jinniPackage', type: 'string', description: 'Package name the adapter\'s device-side half ships as, e.g. "bespok3d-jinni-snapmaker-u1". The app looks the offered version up under this name instead of trusting the version a working copy declares.' },
  { field: 'restartSeconds', type: 'number', description: 'How long this printer takes to answer again after a restart. Every screen that makes the user wait reads it from here, so no number about a printer is written into the app.' },
  { field: 'opSteps', type: 'EnrollStep[]', description: 'Maintenance steps the ops reach for by id (a jinni update, a repair) that are not part of the enroll sequence. Same shape and the same idempotence rule as enrollSteps.' },
  { field: 'lifecycle.deactivate', type: 'EnrollStep[]', description: 'What deactivating does on the printer itself, after the daemon has stopped every plugin: take bespok3d out of the boot sequence and leave the files in place.' },
  { field: 'lifecycle.reactivate', type: 'EnrollStep[]', description: 'What putting a deactivated printer back takes: clear the marker, restore the Klipper and Moonraker include lines, put bespok3d back into the boot sequence and start the daemon. The app then waits for it and re-applies the plugins.' },
  { field: 'lifecycle.remove', type: 'EnrollStep[]', description: 'What a clean uninstall removes, after the daemon has torn every plugin down: every file and every system change enrollment made. Getting this wrong strands the printer, so it is the adapter\'s to own.' },
  { field: 'lifecycle.reboot', type: 'EnrollStep[]', description: 'How this printer is asked to power cycle. The dropped connection is the reboot happening and is swallowed here; the app owns the wait for it to come back.' },
  { field: 'readDaemonLog', type: '(ssh) => Promise<string>', description: 'The tail of the daemon\'s own log on this printer. Shown when a daemon did not come up, so a start failure reads as an import error or a bind failure instead of a bare timeout.' },
  { field: 'workspaceRoot', type: '(ssh) => Promise<string>', description: 'The absolute path of the bespok3d workspace on this printer, asked of a live session because an adapter whose tree lives in the login user\'s home cannot name it before logging in. The app writes exactly one file there itself: the access list, on a reset.' },
  { field: 'diagnoseDaemon', type: '(ssh) => Promise<string>', description: 'Optional. One line of what is wrong with the daemon, shown as a progress hint before a repair. An adapter that does not offer one has the app say so rather than guess with another printer\'s paths.' },
]
