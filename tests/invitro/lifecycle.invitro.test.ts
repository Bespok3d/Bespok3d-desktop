...
describe('in-vitro plugin lifecycle (real daemon-client over the wire)', () => {
  ...
  it('selfcheck reports drift over the wire once an installed symlink is removed', async () => {
    ...
    try {
      ...
      expect(clean.drift ?? []).not.toContainEqual(expect.objectContaining({ plugin_id: 'invitro-selfcheck' }))
      ...
      expect(drifted.drift ?? []).toContainEqual(expect.objectContaining({ plugin_id: 'invitro-selfcheck' }))
    } finally {
