...
// `npm run golden:refresh`: regenerate scripts/test/golden/monorepo/index.json from the manifests the
// sibling plugin repos carry right now.
//
...
async function refreshGolden() {
  ...
  try {
    const { index } = await buildGoldenBundle(outputDir)
    writeGoldenIndex(index)
    console.log(`Refreshed ${GOLDEN_INDEX_PATH}`)
    console.log(`  ${index.plugins.length} plugins, ${index.collections.length} collections, updated ${index.updated}`)
